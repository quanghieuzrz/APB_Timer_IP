...
// 4. TCR Register Logic (Combined into a single synchronous block)
reg [31:0] tcr;
wire timer_en_tmp;
wire div_en_tmp;
wire div_en_err;
...
// Single block for writing to tcr
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        tcr <= 32'h0000_0100;
    else
...
// 9. Read Logic (Reads real-time count value)
reg [31:0] rdata_tmp;
always @(*) begin
    if(rd_en) begin
        case (addr_tmp)
            ADDR_TCR   : rdata_tmp = tcr;
            ADDR_TDR0  : rdata_tmp = count_low;  // Direct read from counter
            ADDR_TDR1  : rdata_tmp = count_high; // Direct read from counter
